...
#pragma warning restore 0219
        ...
        /// <summary>
		/// The flow group auto column-count key selector property.
		/// </summary>
		public static BindableProperty FlowColumnModeProperty = BindableProperty.Create(nameof(FlowColumnMode), typeof(FlowColumnMode), typeof(FlowListView), FlowColumnMode.SingleColumn);

        /// <summary>
        /// Gets or sets FlowListView auto column-count mode.
        /// It defines how many columns should be used 
        /// </summary>
        /// <value>FlowListView auto column-count mode.</value>
        public FlowColumnMode FlowColumnMode
        {
            get { return (FlowColumnMode)GetValue(FlowColumnModeProperty); }
            set { SetValue(FlowColumnModeProperty, value); }
        }
        ...
        public static BindableProperty FlowColumnCountLandscapeProperty = BindableProperty.Create(nameof(FlowColumnCountLandscape), typeof(int?), typeof(FlowListView), default(int?));
        ...
        /// </summary>
        /// <value>The flow column count.</value>
        public int? FlowColumnCountLandscape
        {
            get { return (int?)GetValue(FlowColumnCountLandscapeProperty); }
            set { SetValue(FlowColumnCountLandscapeProperty, value); }
        }
        ...
        /// <summary>
		/// The flow column default maximum width property.
		/// </summary>
		public static BindableProperty FlowColumnMaxWidthProperty = BindableProperty.Create(nameof(FlowColumnMaxWidth), typeof(double), typeof(FlowListView), 50d);

        /// <summary>
        /// Gets or sets the maximum column width of FlowListView.
        /// </summary>
        /// <value>The maximum column width.</value>
        public double FlowColumnMaxWidth
        {
            get { return (double)GetValue(FlowColumnMaxWidthProperty); }
            set { SetValue(FlowColumnMaxWidthProperty, value); }
        }
        ...
        internal FlowColumnMode LastColumnMode { get; set; }
        internal int LastColumnCount { get; set; }
        private void RefreshDesiredColumnCount()
        {

            double listWidth = Math.Max(Math.Max(Width, WidthRequest), MinimumWidthRequest);
            if (FlowColumnMode != FlowColumnMode.SingleColumn && FlowColumnCount.HasValue)
            {
                if (listWidth > 0)
                {
                    double minWidthColumns, maxWidthColumns;
                    switch (FlowColumnMode)
                    {

                        case FlowColumnMode.SetCountWithBounds:
                            minWidthColumns = Math.Floor(listWidth / FlowColumnMinWidth);
                            maxWidthColumns = Math.Ceiling(listWidth / FlowColumnMaxWidth);

                            DesiredColumnCount =
                                (int)Math.Max(Math.Min(minWidthColumns, FlowColumnCount.Value), maxWidthColumns);
                            break;

                        case FlowColumnMode.SetCountWithLowerBound:
                            DesiredColumnCount =
                                (int)Math.Min(Math.Floor(listWidth / FlowColumnMinWidth), FlowColumnCount.Value);
                            break;

                        case FlowColumnMode.SetCountWithUpperBound:
                            DesiredColumnCount =
                                (int)Math.Max(FlowColumnCount.Value, Math.Ceiling(listWidth / FlowColumnMaxWidth));
                            break;

                        case FlowColumnMode.SetCount:
                            DesiredColumnCount = FlowColumnCount.Value;
                            break;

                        case FlowColumnMode.MinimumWidth:
                            DesiredColumnCount = (int)Math.Floor(listWidth / FlowColumnMinWidth);
                            break;

                        case FlowColumnMode.SetCountPerOrientation:
                            if (FlowColumnCountLandscape.HasValue) DesiredColumnCount = Width > Height ? FlowColumnCountLandscape.Value : FlowColumnCount.Value;
                            break;

                        case FlowColumnMode.SetCountPerOrientationWithBounds:
                            minWidthColumns = Math.Floor(listWidth / FlowColumnMinWidth);
                            maxWidthColumns = Math.Ceiling(listWidth / FlowColumnMaxWidth);
                            ...
                            DesiredColumnCount =
                                (int)Math.Max(Math.Min(minWidthColumns, DesiredColumnCount), maxWidthColumns);
                            break;
                    }
                }
                else
                {
                    LastColumnMode = FlowColumnMode;
                    LastColumnCount = FlowColumnCount.Value;
                }
            }
        }
        ...
        private void FlowListSizeChanged(object sender, EventArgs e)
        {
            ...
            if (FlowColumnMode != FlowColumnMode.SingleColumn)
            {
